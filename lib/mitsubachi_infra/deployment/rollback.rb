  ...
  module Deployment
    class Rollback
      ...
      def rollback_backend
        manager = ReleaseManager.new(root: @config.backend_root, runner: @runner, keep: @config.fetch("backend").fetch("keep_releases"))
      ...
      def rollback_frontend
        manager = ReleaseManager.new(root: @config.frontend_root, runner: @runner, keep: @config.fetch("frontend").fetch("keep_releases"))
        target = manager.previous_release
        if target.nil? && @runner.dry_run
