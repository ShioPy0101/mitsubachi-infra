  ...
  class Installer
    ...
    def install_directories
      ...
      %w[backend frontend repositories].each do |dir|
        @runner.run("install", "-d", "-o", @config.fetch("deploy").fetch("user"), "-g", @config.fetch("deploy").fetch("user"), "-m", "0755", File.join(root, dir))
      end
      @runner.run("install", "-d", "-o", "root", "-g", "root", "-m", "0755", "/var/lib/mitsubachi/acme")
      @runner.run("install", "-d", "-o", "root", "-g", @config.fetch("deploy").fetch("user"), "-m", "0750", "/etc/mitsubachi")
    end
    ...
    def install_templates
      ...
      unit = ERB.new(File.read(File.join(@repo_root, "templates", "systemd", "mitsubachi-api.service.erb")), trim_mode: "-").result(binding)
      tmp = "/tmp/mitsubachi-api.service.#{$$}"
      File.write(tmp, unit)
      @runner.run("install", "-o", "root", "-g", "root", "-m", "0644", tmp, "/etc/systemd/system/mitsubachi-api.service")
