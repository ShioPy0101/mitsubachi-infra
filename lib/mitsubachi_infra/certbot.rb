# frozen_string_literal: true

require 'resolv'
require_relative 'errors'

module MitsubachiInfra
  class Certbot
    ACME_ROOT = '/var/lib/mitsubachi/acme'

    def initialize(config:, runner:, nginx:, health:)
      @config = config
      @runner = runner
      @nginx = nginx
      @health = health
    end

    def check
      validate_public_config!
      @config.certificate_domains.each do |host|
        addresses = Resolv.getaddresses(host)
        raise Error, "DNS does not resolve: #{host}" if addresses.empty?
      end

      @runner.run('certbot', 'certificates', allow_failure: true)
    end

    def enable(staging: false)
      validate_public_config!
      email = @config.fetch('https').fetch('email')
      @runner.run('install', '-d', '-o', 'root', '-g', 'root', '-m', '0755', ACME_ROOT)
      @nginx.install(mode: 'public_http_challenge')
      @config.certificate_domains.each do |host|
        if certificate_present?(host)
          @runner.run('certbot', 'certificates', '-d', host, allow_failure: true)
          next
        end

        args = ['certbot', 'certonly', '--webroot', '-w', ACME_ROOT, '-d', host, '--email', email, '--agree-tos',
                '--non-interactive']
        args << '--staging' if staging
        @runner.run(*args, timeout: 1800)
        unless @runner.dry_run || certificate_present?(host)
          raise Error,
                "certificate files are missing after certbot: #{host}"
        end
      end

      @nginx.install(mode: 'public_https')
    end

    def renew
      @runner.run('certbot', 'renew', timeout: 1800)
      @nginx.test!
      @nginx.reload
    end

    def status
      @runner.run('certbot', 'certificates', allow_failure: true)
    end

    private

    def validate_public_config!
      raise Error, 'deployment_mode must be public' unless @config.public?

      @config.validate!
    end

    def certificate_present?(host)
      File.exist?("/etc/letsencrypt/live/#{host}/fullchain.pem") &&
        File.exist?("/etc/letsencrypt/live/#{host}/privkey.pem")
    end
  end
end
