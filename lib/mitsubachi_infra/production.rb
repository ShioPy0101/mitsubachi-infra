...
module MitsubachiInfra
  class Production
    ...
    def rollback(target)
      check_server_id
      case target
      when "backend" then rollback_root(@config.fetch("paths").fetch("rails_root"), restart: [API_SERVICE, JOBS_SERVICE])
    ...
    def production_check
      ...
      @logger.puts("[LOCAL] Caddy active: #{privileged_success?("systemctl", "is-active", "--quiet", "caddy") ? "yes" : "no"}")
      @logger.puts("[LOCAL] Rails API service: #{privileged_success?("systemctl", "is-active", "--quiet", API_SERVICE) ? "active" : "inactive"}")
      @logger.puts("[LOCAL] Solid Queue worker: #{privileged_success?("systemctl", "is-active", "--quiet", JOBS_SERVICE) ? "active" : "inactive"}")
      @logger.puts("[LOCAL] Caddyfile validation: #{privileged_success?("caddy", "validate", "--config", @config.fetch("paths").fetch("caddyfile")) ? "ok" : "failed"}")
    ...
    def deploy_backend(ref: nil)
      ...
      release = create_git_release(root: root, repository: app.fetch("repository"), ref: ref || app.fetch("ref"), label: "backend")
    ...
    def deploy_frontend(ref: nil)
      ...
      release = create_git_release(root: root, repository: app.fetch("repository"), ref: ref || app.fetch("ref"), label: "frontend")
      begin
        @runner.deploy("npm", "ci", config: deploy_config, chdir: release, timeout: 1800)
        @runner.deploy(*app.fetch("build_command"), config: deploy_config, chdir: release, timeout: 1800, env: frontend_env)
    ...
    def create_git_release(root:, repository:, ref:, label:)
      ...
      sha = @runner.deploy("git", "rev-parse", "--verify", "FETCH_HEAD^{commit}", config: deploy_config, chdir: release).stdout.strip
    ...
    def install_backend(release)
      ...
      @runner.deploy("bundle", "config", "set", "--local", "without", "development test", config: deploy_config, chdir: release)
      ...
      @runner.deploy("bundle", "exec", "rails", "runner", "abort 'queue adapter is not solid_queue' unless Rails.application.config.active_job.queue_adapter.to_s == 'solid_queue'", config: deploy_config, chdir: release)
    end
    ...
    def install_bundler(release)
      ...
      return if @runner.deploy("gem", "list", "--installed", "--exact", "bundler", "--version", version, config: deploy_config, allow_failure: true).success?
    ...
    def ensure_deploy_user
      ...
      privileged("useradd", "--system", "--user-group", "--create-home", "--home-dir", "/home/#{deploy_user}", "--shell", "/bin/bash", deploy_user)
    ...
    def install_directories
      [@config.fetch("paths").fetch("rails_root"), @config.fetch("paths").fetch("frontend_root")].each do |root|
        privileged("install", "-d", "-o", deploy_user, "-g", deploy_user, "-m", "0755", root, File.join(root, "releases"), File.join(root, "shared"))
      end
    ...
    def install_env_files
      ...
      atomic_write(rails_path, EnvTemplates.rails_env(@config), owner: "root", group: deploy_user, mode: "0640") unless File.exist?(rails_path) || @runner.dry_run
    ...
    def install_systemd_units
      {
        ...
        content = ERB.new(File.read(File.join(@repo_root, "templates", "systemd", template)), trim_mode: "-").result(binding)
    ...
    def check_server_id(allow_create: false)
      ...
      raise ValidationError, "server ID mismatch: expected #{expected}, got #{actual.empty? ? "(missing)" : actual}" unless actual == expected
    end

    def ensure_required_rails_env!
      path = @config.fetch("paths").fetch("rails_env")
      text = File.exist?(path) ? File.read(path) : ""
      required = %w[DATABASE_URL DATABASE_CACHE_URL DATABASE_QUEUE_URL DATABASE_CABLE_URL RAILS_MASTER_KEY SECRET_KEY_BASE RESEND_API_KEY MAIL_FROM]
    ...
    def activate(root, release)
      tmp = File.join(root, ".current.tmp.#{$$}")
    ...
    def cleanup(root, keep:, protected_paths: [])
      protected = ([current_release(root)] + protected_paths).compact.map { |path| File.realpath(path) rescue path }
    ...
    def release_paths(root)
      Dir.glob(File.join(root, "releases", "*")).select { |path| File.directory?(path) }.sort_by { |path| File.mtime(path) }
    end
    ...
    def atomic_write(path, content, owner:, group:, mode:)
      ...
      tmp = File.join(Dir.tmpdir, ".#{File.basename(path)}.#{$$}.tmp")
    ...
    def warn_dirty_infra_tree
      ...
      @logger.puts("[LOCAL] warning: mitsubachi-infra working tree has uncommitted changes") unless result.stdout.strip.empty?
    end
    ...
    def release_name(label)
      "#{Time.now.utc.strftime("%Y%m%dT%H%M%SZ")}-#{label}-#{$$}-#{SecureRandom.hex(3)}"
    end
