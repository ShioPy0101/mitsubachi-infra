...
module MitsubachiInfra
  class CLI
    ...
    def install
      ...
      locked { Installer.new(config: @config, runner: @runner, repo_root: @repo_root).install(interactive: local[:interactive]) }
    end
    ...
    def deploy
      ...
      locked do
        ...
        case target
        when "backend" then Deployment::Backend.new(config: @config, runner: @runner, systemd: systemd, health: health).deploy(ref: opts[:ref])
        when "frontend" then Deployment::Frontend.new(config: @config, runner: @runner, health: health).deploy(ref: opts[:ref])
        when "all"
          Deployment::Backend.new(config: @config, runner: @runner, systemd: systemd, health: health).deploy(ref: opts[:backend_ref] || opts[:ref])
          Deployment::Frontend.new(config: @config, runner: @runner, health: health).deploy(ref: opts[:frontend_ref] || opts[:ref])
    ...
    def status
      ...
      Status.new(config: @config, runner: @runner, systemd: Systemd.new(runner: @runner), health: HealthCheck.new(logger: $stderr)).print(json: opts[:json])
    end
