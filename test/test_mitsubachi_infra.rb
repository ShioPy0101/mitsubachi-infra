...
class MitsubachiInfraTest < Minitest::Test
  ...
  def production_config(root)
    MitsubachiInfra::Configuration.new("/missing", data: config_data(
      ...
      "deploy" => MitsubachiInfra::Configuration::DEFAULT.fetch("deploy").merge("app_root" => File.join(root, "legacy")),
      "server" => {
        "deploy_user" => "deploy",
  ...
  def test_systemd_worker_template_uses_bin_jobs_and_rails_env
    Dir.mktmpdir do |dir|
      config = production_config(dir)
      @config = config
      rendered = ERB.new(File.read(File.join(ROOT, "templates", "systemd", "mitsubachi-jobs.service.erb")), trim_mode: "-").result(binding)
